...
function StatsStrip({ total, byCategory }: { total: number; byCategory: StatsResponse["by_category"] }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-4 gap-px border border-[var(--color-border-subtle)] rounded overflow-hidden">
      ...
      {byCategory.map((c) => (
        <div key={c.category} className="bg-[var(--color-surface)] px-6 py-5 border-l border-[var(--color-border-subtle)]">
...
export default function OverviewTab() {
  ...
  if (error) {
    return (
      <div className="border border-[var(--color-danger)] bg-[var(--color-danger-soft)] rounded px-4 py-3 text-sm font-sans text-[var(--color-danger)]">
