...
Add a public "Partenaires & Sources de données" page to the marches-publics frontend. It shows site visitors the ecosystem behind the product: the official Moroccan data portals the app draws on (with real, probe-verified facts) and the strategic partner institutions. It is an ecosystem/transparency page, not an internal dashboard: internal probe judgments (scrape feasibility, WAF blockers, partnership tactics) are excluded.

## Data sources (inputs)

- `scraping/source_inventory.jsonl` — 17 probed official portals (2026-07-15 reconnaissance). Canonical source; the TS file below is its curated public projection.
...
export interface DataSource {
  ...
  status: "integre" | "disponible" | "partenariat";
}
...
- `dataSources`: all 17 portals. Status assignment: `integre` = PMMP only (already harvested by `backend/scraper.py`); `partenariat` = Directinfo/OMPIC, Rokhas, DGI, ADII (probe found CAPTCHA/WAF/auth barriers); `disponible` = the rest.
...
1. **Header** — title "Partenaires & Sources de données" + one intro sentence (the app is built on official public Moroccan sources).
2. **Sources de données officielles** — cards grouped by tier with French group headings:
   ...
   Each card: portal name, operator, domain badges, description, status badge (Intégré / Disponible / Partenariat), external link to the official portal.
3. **Partenaires stratégiques** — card grid: name, role sentence, tags, official link.
4. **Attribution note** — closing paragraph: data originates from official public portals, deep links to originals, retrieval-based.
